...
import com.begcode.report.core.expression.ErrorInfo;
import com.begcode.report.core.expression.ScriptErrorListener;
...
public class DesignerController extends AbstractReportBasicController {
    ...
    public R loadReportTable(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            return R.ok().success(Optional.ofNullable(reportDefinition).map(ReportDefinitionWrapper::new).orElse(null));
        } catch (Exception e) {
            e.printStackTrace();
